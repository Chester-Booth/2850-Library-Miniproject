...
fun addBooks(filePath: String): ISBNToIdMap {
    FileReader(filePath).use { reader ->
        // remove 'duplicates'
        val records = CSVFormat.DEFAULT.parse(reader).drop(1).distinctBy { it[2] } 
        val books = ISBNToIdMap()
        for (record in records) {
            if (record[2].isEmpty()) { continue } // skip entries without and ISBN
            val isbn = record[2].toDouble().toLong()
            books[isbn] = BooksTable.insertAndGetId {
                it[title] = record[0]
                it[author] = record[1]
                it[bookISBN] = isbn
                it[details] = null
...
fun addCopies(filePath: String, books: ISBNToIdMap) {
    FileReader(filePath).use { reader ->
        val records = CSVFormat.DEFAULT.parse(reader).drop(1)
        for (record in records) {
            if (record[2].isEmpty()) { continue }
            val isbn = record[2].toDouble().toLong()
